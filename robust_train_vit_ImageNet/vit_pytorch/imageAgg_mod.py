...
class Transformer(nn.Module):
    def __init__(self, dim, seq_len, depth, heads, mlp_mult, dropout = 0.):
        ...
        #         PreNorm(dim, FeedForward_conv(dim, mlp_mult, dropout = dropout))
        #     ]))

        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Attention(dim, heads = heads, dropout = dropout),
                FeedForward_conv(dim, mlp_mult, dropout = dropout)
            ]))
    ...
    def forward(self, x):
        *_, h, w = x.shape
        ...
        pos_emb = self.pos_emb[:(h)]
        pos_emb = rearrange(pos_emb, '(h) -> () h ()', h = h)
...
class ImageAgg(nn.Module):
    def __init__(
        ...
        *,
        ...
        # )
        self.mlp_head_ori = nn.Sequential(
            nn.LayerNorm(dim),
            Reduce('b d c -> b c', 'mean'),
            nn.Linear(dim, num_classes)
        )
    ...
    def forward(self, img):
        ...
        # b, c, h, w = x.shape
        # x = self.conv(img)
        x = self.to_patch_embedding3(img)
        ...
        for level, (transformer, aggregate) in zip(reversed(range(num_hierarchies)), self.layers):
            ...
            x = rearrange(x, 'b (a b1 b2) c -> (b b1 b2) a c', b1 = block_size, b2 = block_size)
            x = transformer(x)
