...
from engine import train_one_epoch, evaluate, robust_evaluate, LabelSmoothCELoss
...
def get_args_parser():
    ...
    parser.add_argument('--mixup', type=float, default=0.2,
                        help='mixup alpha, mixup enabled if > 0. (default: 0.8), 0.2 for TPAMI paper')
...
def main(args):
    ...
    output_dir_all = args.output_dir + '/imagenet/' + args.embedding + '-' + args.Transblock + '-' + args.MLP + '-' +  args.norm + '-' + args.skipconnect + '/'
    ...
    criterion = LabelSmoothCELoss(args.smoothing, 1000)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.advtrain:
            ...
            if max_avg < (test_stats["acc1"] + robust_test_stats["acc1"])/2:
                max_accuracy = test_stats["acc1"]
                max_robust_accuracy = robust_test_stats["acc1"]
                max_avg = (max_accuracy + max_robust_accuracy)/2
                if args.output_dir:
                    checkpoint_paths = [output_dir / 'best_checkpoint.pth']
                    for checkpoint_path in checkpoint_paths:
                        utils.save_on_master({
